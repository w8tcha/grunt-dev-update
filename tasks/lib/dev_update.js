...
module.exports = function (grunt) {
    ...
    var getPkgJsonPath = function () {
        ...
        if (exports.options.packageJson) {
            grunt.verbose.writelns(
                `Using custom option for package.json: ${exports.options.packageJson}`
            );
        ...
        forEach(packages, function (dep) {
            ...
            forEach(dep.deps, function (item, key) {
                ...
                if (!includes(["version", "tag", "range"], parsed.type)) {
                    grunt.verbose.writelns(
                        key.red + " - doesn't seem local to npm. Skipping..."
                    );
                    return null;
                }
                mappedPkgs.push({
    ...
    var processByUpdateType = function (pkg, specs, done) {
        ...
        grunt.log.writelns(
            "Current version\t:",
            (specs.current && specs.current.green) || "<not installed>"
        );
        ...
        const onlyReportPkg = shouldOnlyReport(
            exports.options.reportOnlyPkgs,
            pkg.name
        );
        ...
        if (updateType === "fail" && !onlyReportPkg) {
            grunt.warn(
                `Found an outdated package: ${String(pkg.name).underline}.`,
                3
            );
        }
    ...
    exports.runTask = function (options, done) {
        ...
        const packageTypes = filter([devDeps, prodDeps], function (pkgType) {
        ...
        getOutdatedPkgs(packages, function (err, result) {
            ...
            asyncEach(
                ...
                function (pkgName, cb) {
                    ...
                    if (
                        !semver.valid(specs.wanted) ||
                        !semver.valid(specs.latest)
                    ) {
                        grunt.verbose.writelns(
