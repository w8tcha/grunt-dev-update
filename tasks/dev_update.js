...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask(
        "devUpdate",
        "See your outdated devDependencies and update them",
        function () {
            //set default options
            ...
            if (!includes(possibleUpdateTypes, updateType)) {
                grunt.warn(
                    `updateType ${String(updateType).cyan} not supported.`
                );
                //if force
            ...
            if (updateType === "force") {
                ...
                grunt.log.writelns(
                    `Running with update type of ${"force".red}`
                );
            }
            ...
            if (typeof _pkgjson === "string") {
                ...
                if (!grunt.file.exists(_pkgjson)) {
                    grunt.warn(
                        `Cannot locate package.json in supplied path ${_pkgjson}`
                    );
                    //if force
